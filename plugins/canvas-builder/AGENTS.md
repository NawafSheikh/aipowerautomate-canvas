# AGENTS.md — canvas-builder Plugin

This file provides guidance to AI Agents when working with the **canvas-builder** plugin.

## What This Plugin Is

A plugin for authoring Power Apps Canvas Apps. The canvas-builder MCP server exposes tools that agents use to generate, validate, and compile Canvas App YAML files (`.pa.yaml`) in conjunction with a running coauthoring studio session. The Power Apps Studio browser tab must remain open for the duration of the session — closing it ends the coauthoring session, which breaks `compile_canvas` and `sync_canvas` operations.
...
skills/
  configure-canvas-builder/
    SKILL.md                   ← Registers the canvas-builder MCP server with Claude Code
  generate-canvas-app/
...
| Skill | Description |
|-------|-------------|
| `/configure-canvas-builder` | Register the canvas-builder MCP server with Claude Code |
...
The `canvas-builder` MCP server exposes the following tools:
