---
name: configure-canvas-builder
version: 1.0.0
description: Configure the canvas-builder MCP server for Claude Code, VS Code Copilot, or GitHub Copilot CLI. USE WHEN "configure MCP", "set up MCP server", "MCP not working", "connect Canvas Apps MCP", "canvas-builder not available", "MCP not configured", "set up canvas apps". DO NOT USE WHEN prerequisites are missing — direct the user to install .NET 10 SDK first.
author: NawafSheikh
user-invocable: true
allowed-tools: Bash, AskUserQuestion, Read, Edit, Write
---

# Configure the canvas-builder MCP Server

This skill registers the canvas-builder MCP server with Claude Code, VS Code Copilot, or GitHub Copilot CLI using the user's Power Platform environment ID.

## Instructions

### 0. Check prerequisites

Verify Node.js 18+ and .NET 10 SDK are installed.
...
> ⚠️ Node.js 18+ is required to run the canvas-builder MCP server. Install from https://nodejs.org and re-run this skill.

If a .NET SDK version 10.x.y or higher is not listed:

> ⚠️ The canvas-builder authoring runtime requires .NET 10 SDK. Install via `winget install Microsoft.DotNet.SDK.10` (Windows) or https://dotnet.microsoft.com/download/dotnet/10.0, then re-run this skill.

Wait for both prerequisites before continuing.
...
> Which tool would you like to configure the canvas-builder MCP server for?
> 1. **VS Code Copilot**
...
>
> Copy the URL from the browser address bar while your app is open in Power Apps Studio (it should look like `https://make.powerapps.com/e/Default-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/canvas/?action=edit&app-id=...`).
>
...
```bash
claude mcp add --scope {CLAUDE_SCOPE} canvas-builder \
...
```bash
claude mcp remove canvas-builder
claude mcp add --scope {CLAUDE_SCOPE} canvas-builder \
  -e CANVAS_ENVIRONMENT_ID={ENV_ID} \
  -e CANVAS_APP_ID={APP_ID} \
  -e CANVAS_CLUSTER_CATEGORY={CLUSTER_CATEGORY} \
...
> ✅ canvas-builder MCP server configured (`canvas-builder`, scope: `{CLAUDE_SCOPE}`).
>
...
> - `canvas-builder` should appear in the MCP server list
...
> ✅ canvas-builder MCP server configured (`canvas-builder`, configPath: `{CONFIG_PATH}`).
>
